  ...
  return (
    <div className="grid lg:grid-cols-2 xl:grid-cols-3 gap-5 w-full">
        {results.map((result) => 
            ...
            >
                <img className="object-contain w-full h-40 py-5" srcSet={result.imageset} alt={result.title} />
                <div className="flex flex-col py-5 flex-1">
                    <p className="font-bolt">{result.title}</p>
                    <p className="text-sm text-gray-500"> {result.rating} ({result.reviews} reviews)</p>
                
                    <div className="flex space-x-2 justify-end flex-1">
                        <p className="font-bolt text-indigo-500 pt-2 text-xl mt-auto">
                            {result.price > 0 ? `$${result.price}`  : "N/A"}
                        </p>

                        {result.previous_price > 0 && (
                            <p className="font-bolt text-indigo-500/50 line-through pt-2 text-xl mt-auto">
                    ...
                    <div className="flex flex-wrap gap-2 justify-end mt-5">
                        {result.features.map((feature) => (
                            feature && 
                            <p key={feature} className="text-xs bg-indigo-500 px-2 py-1 text-white rounded-md"> {feature} </p>
